...
namespace Poker
{
    ...
    public class Combination
    {
        ...
        public override string ToString()
        {
            ...
            switch (combination)
            {
                ...
                case Combinations.HighCard: { result = "старшую карту " + cards[0].Rank.ToString(); break; }
                case Combinations.Pair: { result = "пару " + cards[0].Rank.ToString(); break; }
                case Combinations.TwoPairs: { result = "две пары: " + cards[0].Rank.ToString() + " и " + cards[2].Rank.ToString(); break; }
                case Combinations.Tree: { result = "тройку " + cards[0].Rank.ToString(); break; }
                case Combinations.Streight: { result = "стрит " + cards[0].Rank.ToString(); break; }
                case Combinations.FullHouse: { result = "фул хаус: " + cards[0].Rank.ToString() + " и " + cards[4].Rank.ToString(); break; }
                case Combinations.Flash: { result = "флеш " + cards[0].ToString(); break; }
                case Combinations.Kare: { result = "каре " + cards[0].Rank.ToString(); break; }
                case Combinations.StreightFlash: { result = "стрит-флеш " + cards[0].ToString(); break; }
    ...
    public class Record
    {
        ...
        public override string ToString()
        {
            ...
            switch ((int)move)
            {
                case 0: { record += player.Name + " сбросил;"; break; }
                case 1: { record += player.Name + " проверяет;"; break; }
                case 2: { record += player.Name + " поддерживает до $" + player.stepBet + " ;"; break; }
                case 3: { record += player.Name + " поднимает до $" + player.stepBet + " ;"; break; }
                case 4: { record += player.Name + " выигрывает $" + sum + ";"; break; }
                case 5: { record += "-------Новая раздача!-------"; break; }
                case 6: { record += player.Name + " поставил малый блайнд $" + sum + " ;"; break; }
                case 7: { record += player.Name + " поставил большой блайнд $" + sum + " ;"; break; }
                case 8: { record += player.Name + " выигрывает $" + sum + ", собрав " + player.cards.ToString(); break; }
                case 9: { record += "-------Новый раунд торговли!-------"; break; }
            }
            return record;
        }
